...
describe BinarySearchTree do
  ...
  describe '#pre_order_traversal' do
    it 'visits self, then left children, then right children' do
      in_order_array = [5, 3, 1, 0, 2, 1.5, 4, 7, 9, 10]
      expect(prefilled_bst.pre_order_traversal).to eq(in_order_array)
    end
  end

  describe '#post_order_traversal' do
    it 'visits left children, then right children, then self' do
      in_order_array = [0,1.5,2,1,4,3,10,9,7,5]
      expect(prefilled_bst.post_order_traversal).to eq(in_order_array)
    end
  end

  describe '#reverse_in_order_traversal' do
    it 'visits right children, then self, then left children' do
      in_order_array = [10,9,7,5,4,3,2,1.5,1,0]
      expect(prefilled_bst.reverse_in_order_traversal).to eq(in_order_array)
    end
  end
end